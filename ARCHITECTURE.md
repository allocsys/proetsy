...
**Build sequence.** Nothing beyond the DB schema (`image_preferences`, `taste_centroids`, `prompt_terms` — already in `backend/db/schema.sql`) exists yet for this module. Broken into independently-committable sub-steps, mirroring Module 3's AI-outpainting build log above:
1. **Embeddings helper — in progress.** `backend/lib/taste-filter/embeddings.js`: a pure `preprocessImage()` (resize/center-crop to 224x224, RGB channel split, CLIP mean/std normalization — unit-testable without a real model or network call) and `embedImage(imagePath)`, which loads `Xenova/clip-vit-base-patch32` via `onnxruntime-web` (WASM) and returns an L2-normalized embedding vector. Adds `onnxruntime-web` to `backend/package.json`. The `.onnx` weights file itself is a large binary and isn't committed to the repo — resolved via a configurable model path (env-driven, same pattern as `MOCKUP_TEMPLATES_DIR`), with a clear startup/first-run error if it's missing, not a silent failure.
2. **Persistence + centroid math — not started.** `backend/lib/taste-filter/store.js` (CRUD against `image_preferences`) and `centroids.js` (pure functions: compute a kept/discarded centroid pair from a set of embeddings, both global and per-category, matching the product-sizes categories).
3. **Scoring — not started.** Pure function scoring a candidate embedding against both centroid pairs (global score, per-category score) per the two-scores design above.
4. **Routes — not started.** Batch import, label (keep/discard), list a scored batch, "Recompute now".
5. **Dashboard UI — not started.** Ranked batch grid (likely-keep/likely-discard/uncertain), keep/discard buttons, side-by-side global/category scores.
6. **Prompt-feedback link to Module 4 (write side) — not started.** Module 4 already reads from `prompt_terms` (see Module 4's status note); this step is the write side — tallying kept/discarded counts per prompt term as labels come in.
7. **Auto-import via watched folder — future, deferred per the section below, not part of this sequence.**
8. **Tests — ongoing alongside each step above**, not a separate final pass (matches how Module 3's step 8 only covered what wasn't already tested inline).
