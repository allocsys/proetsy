...
**Backoff on full exhaustion.** If a cascade sweep still ends with every `(key, model)` pair either 429'ing live or already in cooldown, that's treated as possibly transient (e.g. a short burst across the whole pool) rather than an immediate hard failure:

- The provider layer retries the **entire sweep** with **exponential backoff + jitter** (`LLM_BACKOFF_BASE_MS`, e.g. 1s → 2s → 4s, capped, plus randomized jitter to avoid synchronized retries), up to `LLM_MAX_SWEEP_RETRIES` (e.g. 2 extra sweeps) before finally throwing
- Each retried sweep re-checks the cooldown cache, so a pair whose cooldown expires mid-backoff becomes eligible again on the next sweep without any special-casing
- Only the *sweep-level* retry backs off like this — within a single sweep, cascading from model to model / key to key is still immediate, since those are different targets, not retries of the same one
- This is bounded and config-driven specifically so it can't turn into a silent hang: worst case is a small, known number of backed-off sweeps before the existing "fails clearly, surfaced in the dashboard" behavior kicks in
