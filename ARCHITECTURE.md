...
2. **Packaged-mode frontend loading — not yet done.** `main.js` currently throws explicitly when `app.isPackaged` is true rather than silently pointing a packaged build at a dev server that won't exist. Needs: loading `frontend/dist/index.html` from packaged resources, and pointing `DB_PATH`/`ARTWORK_UPLOADS_DIR`/`TASTE_FILTER_CANDIDATES_DIR`/mockup `OUTPUT_DIR` (all already env-overridable — see Database Schema and the relevant module sections) at a writable location such as `app.getPath('userData')`, since a packaged app's install directory (e.g. `Program Files` on Windows) isn't writable.
3. **`electron-builder` config — not yet done.** Add `electron-builder` as a devDependency and a Windows (NSIS) build target, bundling backend source (minus `node_modules`/local `data/`) + the built `frontend/dist` — the same asset set `release.yml`'s existing `package-local-bundle` job already assembles for the tarball release, just packaged as an installer instead of a tarball to extract manually.
4. **`better-sqlite3` native-module ABI handling — not yet done, flagged as a real risk.** Sub-step 1's system-`node` spawn sidesteps this in dev (the backend runs against whatever Node ABI is already on the dev machine, same as running it directly), but a packaged app can't assume a system-wide `node` binary exists on the target machine's PATH. The standard fix — spawning via `process.execPath` with `ELECTRON_RUN_AS_NODE=1` (Electron's own bundled Node) instead of a system `node`, plus rebuilding `better-sqlite3` against Electron's ABI via `@electron/rebuild` as part of the packaging step — needs to land alongside sub-step 3, not after it, since an unrebuilt native module will fail at require-time in the packaged app.
5. **Wire into `release.yml` — not yet done.** Replace the `TODO(step 6)` comment in that workflow's header with the real `electron-builder --publish always` step once sub-steps 2-4 work locally, producing the actual Windows installer/exe as a release artifact instead of (or alongside) the current local-bundle tarball.
