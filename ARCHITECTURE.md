...
**Not yet fixed:** this needs `npm install` run with real network/registry access to regenerate `package-lock.json` against the current `package.json`s, then committing the updated lockfile — not something that can be done by hand-editing the lockfile's resolved-version/integrity entries. Blocked on that regeneration step; the asar-contents/`dependencies`-field question from the fourth run is consequently still unverified too, one step further back than it looked after run #4.
