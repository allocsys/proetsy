...
**CD (packaging, not deployment) — ✅ done.** `.github/workflows/release.yml` triggers on `v*.*.*` tag pushes: a `verify` job re-runs the same lint+test+build gate CI does (duplicated rather than `needs: ci`, since CI's own triggers don't include tag pushes), then two packaging jobs run in parallel — `package-local-bundle` builds the frontend and publishes a tarball (backend source minus `node_modules`/local `data/`, the built `frontend/dist`, and the root config/docs files) as a GitHub Release artifact, enough for a local `npm ci && npm start` deployment without cloning the repo; and `package-electron-win` runs `electron-builder` on `windows-latest` to produce a Windows NSIS installer (`ProEtsy Setup <version>.exe`), rebuilding `better-sqlite3` against Electron's Node ABI via `@electron/rebuild` along the way. Both jobs publish their artifact to the same tagged GitHub Release (gated to `refs/tags/*`, so a `workflow_dispatch` sanity-check run never publishes). The electron-win path was the harder of the two to get right — see "Electron packaging — build sequence" above for the five failed/fixed CI runs (lockfile drift, missing `version`/`author` fields, electron-builder's own publish-metadata step, and the root-`package.json`-`dependencies` gap that kept backend's runtime deps out of the packaged app) that preceded run #6's clean, fully-verified success.
