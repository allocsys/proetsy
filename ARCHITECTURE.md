...
**Tech:** local image embeddings via a **JS-only CLIP implementation** — `onnxruntime-node` running a pre-converted CLIP ONNX model, called directly from the Node backend (not a child process, not a separate Python script). No API key, no per-request cost, no network dependency, and no second runtime to manage — everything (backend, frontend, embeddings) stays in one Node process. This choice matters for later packaging: it means the app can be bundled into a single Windows executable without also bundling a Python runtime.
...
- **Local embedding model:** `onnxruntime-node` running a pre-converted CLIP ONNX model, for Module 7. JS-only — no Python runtime, no API key, no cost, no network call. Runs in-process within the Node backend, not a separate child process.
