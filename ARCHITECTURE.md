...
6. **Prompt-feedback link to Module 4 (write side) — not started.** Module 4 already reads from `prompt_terms` (see Module 4's status note); this step is the write side — tallying kept/discarded counts per prompt term as labels come in. The routes above already thread a `prompt_id` through import → label, so the plumbing to reach a prompt's term list is in place; the tally-on-label logic itself isn't written yet.
7. **Auto-import via watched folder — future, deferred per the section below, not part of this sequence.**
8. **Tests — ongoing alongside each step above**, not a separate final pass (matches how Module 3's step 8 only covered what wasn't already tested inline). Remaining: step 6's write-side logic, once built.
