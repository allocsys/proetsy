 ...
 * @param {string} params.orientation - desired print orientation (e.g. 'portrait', 'landscape', 'square') — drives the --ar flag
 * @param {string[]} [params.styleHints] - optional "terms that have worked well" from Module 7's kept/discard tally (prompt_terms table), when any exist yet — see ARCHITECTURE.md -> Module 7 -> "Prompt-feedback link to Module 4": a style hint only, never overriding the trend/orientation selection itself
 * @returns {string}
 */
export function buildPromptHelperPrompt({ trend, orientation, styleHints = [] }) {
  const MIDJOURNEY_CONVENTIONS = getShopConventions().midjourney;
  const aspectRatio = MIDJOURNEY_CONVENTIONS.aspectRatioByOrientation[orientation] || null;

  const trendLine = trend
    ? `Selected trend/theme: "${trend.term}"${trend.category ? ` (trend category: ${trend.category})` : ''}.`
    : 'No specific trend was selected — generate broadly appealing fine-art print concepts for this orientation.';

  const hintsLine = styleHints.length
    ? `Style hint (optional, from this shop's own history of kept vs. discarded images — lean toward these where they fit naturally, but do NOT let them override the trend or orientation above): ${styleHints.join(', ')}.`
    : '';
  ...
  return [
    ...
    '',
    `Generate exactly ${PROMPT_COUNT} distinct prompt variations — different compositions, angles, or framings on the trend and orientation above, not just reworded restatements of each other.`,
    '',
